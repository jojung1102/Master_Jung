...
% Integrate in time
[t, X_pt] = ode45(MySystem{2}, [0 100], [A_I_Init, A_cont_Init], OPTIONS, C0, zeta, kappa);
...
opt = contset;
opt = contset(opt, 'MaxNumPoints', 1e5);
...
for i = 1:length(branching_points)
    ...
    [X0_side, ~] = init_BP_EP(@Bending_system_2D, X_combined(1:2, branching_points(i).index), p, branching_points(i), 0.0001);
        

    %opt = contset(opt, 'MaxNumPoints', 1e3);
...
%%
% Bifurcation diagram for C_0 = 0.8
% Initial conditions
A_I_Init = 0.5;
A_cont_Init = 0.25;
kappa = 20;
...
% Save combined branch
X_combined_positive = real(X_combined(:, X_combined(3, :) >= 0));
csvwrite(fullfile(savePath, 'Main_branch_C_0.8.csv'), X_combined_positive');
...
for i = 1:length(branching_points)
    ...
    % Save branch
    X_side_combined_positive = real(X_side_combined(:, X_side_combined(3, :) >= 0));
    csvwrite(fullfile(savePath, sprintf('Side_branch_C_0.8_%d.csv', branch)), X_side_combined_positive');
    
    branch = branch + 1;
end

%%
% Bifurcation diagram for C_0 = 1
% Initial conditions
A_I_Init = 0.5;
A_cont_Init = 0.25;
kappa = 20;
...
% Save combined branch
X_combined_positive = real(X_combined(:, X_combined(3, :) >= -1));
csvwrite(fullfile(savePath, 'Main_branch_C_1.csv'), X_combined_positive');
...
for i = 1:length(branching_points)
    ...
    % Save branch
    X_side_combined_positive = real(X_side_combined(:, X_side_combined(3, :) >= -1));
    csvwrite(fullfile(savePath, sprintf('Side_branch_C_1_%d.csv', branch)), X_side_combined_positive');
    
    branch = branch + 1;
end

%%
% Bifurcation diagram for kappa = 30
% Initial conditions
A_I_Init = 0.5;
A_cont_Init = 0.25;
kappa = 10;
...
% Save combined branch
X_combined_positive = real(X_combined(:, X_combined(3, :) >= 0));
csvwrite(fullfile(savePath, 'Main_branch_kappa_10.csv'), X_combined_positive');
...
for i = 1:length(branching_points)
    ...
    % Save branch
    X_side_combined_positive = real(X_side_combined(:, X_side_combined(3, :) >= 0));
    csvwrite(fullfile(savePath, sprintf('Side_branch_kappa_10_%d.csv', branch)), X_side_combined_positive');
