...
outputFolder = [directory '\output\Bifurcations_particles'];
...
particle_frac = 1/10; %a/A
kappa = 20*particle_frac;  % For more realistic values, the branches change their position too much such that they are not found anymore
C0 = 0.4;
n = 0.1;
n_f = n/2;
n_d = n/2;
...
branch = 1;
for i = 1:size(selectedRows, 1)
    X_combined = branch_continuation(0.3, selectedRows(i, 1), selectedRows(i, 2), particle_frac*selectedRows(i, 3));
    csvwrite(fullfile(savePath, sprintf('Branch_c_0.3_%d.csv', branch)), X_combined');
    branch = branch + 1;
end

%% Functions
% Equilibrium conditions for c = 0
function [n_fI, n_dI] = equi(A_I)
    global	e n_f n_d

    gamma = 1 / (1 + exp(-e));

    n_fI = ((A_I + n_f) * (1 - gamma) + gamma - ...
        sqrt(-4 * A_I * n_f * (1 - gamma) + ((A_I + n_f) * (1 - gamma) + gamma).^2)) / (2 * (1 - gamma));

    n_dI = (-A_I * n_d + (A_I * n_d) / (2 * (1 - gamma)) ...
        + (n_f * n_d) / (2 * (1 - gamma)) + (n_d * gamma) / (2 * (1 - gamma)) ...
        - (A_I * n_d * gamma) / (2 * (1 - gamma)) - (n_f * n_d * gamma) / (2 * (1 - gamma)) ...
        - (n_d * sqrt(-4 * A_I * n_f * (1 - gamma) ...
        + (A_I + n_f + gamma - A_I * gamma - n_f * gamma).^2)) / (2 * (1 - gamma))) / (n_f - 1);
end

% Continuation in c, then in zeta
function X_combined = branch_continuation(c, A_I_Init, A_cont_Init, zeta)
    global kappa C0 n_f n_d e
    % Initial variables & parameters
    [n_fI_Init, n_dI_Init] = equi(A_I_Init);
    p = [C0; 0; zeta; kappa; n_f; n_d; e]; % Parameters
    ap = [2];  % Active parameter in continuation
    
    % Initial state values (vertical vector)
    X1 = [A_I_Init; A_cont_Init; n_fI_Init; n_dI_Init];
    ...
    A_I_Init = X_combined(1, idx);
    A_cont_Init = X_combined(2, idx);
    n_fI_Init= X_combined(3, idx);
    n_dI_Init = X_combined(4, idx);
    p(2) = c;
